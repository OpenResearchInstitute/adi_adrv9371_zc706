...
/* READ_FILE For reading BBFrame File  */
/* WORD For sending one word at a time */
#define READ_FILE
...
int main (__notused int argc, __notused char **argv)
{
...
#ifdef READ_FILE
    FILE *fp;
    if (( fp = fopen ( "/media/sd-mmcblk0p1/data/test.bbf", "rb" )) == NULL )
    {
        printf ("Cannot open file test.bbf\n");
        perror("Could not open the BBFrame data file");
        shutdown();
    }
    printf("* Opened the BBFrame data file!\n");

    size_t bbframe_item_size = 4 + 384;
#endif
	...
	while (!stop)
	{
		...
		for (p_dat = iio_buffer_first(txbuf, tx0_i); p_dat < p_end; p_dat += p_inc) {
			...
			((int16_t*)p_dat)[0] = 0x0109; // Real (I)
			((int16_t*)p_dat)[1] = 0x0203; // Imag (Q)
		}
		...
		if (nbytes_tx < 0) { printf("Error pushing buf %d\n", (int) nbytes_tx); shutdown(); }
	       ...
	       // WRITE: Get pointers to TX buf and write IQ to TX buf port 0
	        ...
	        items_read = fread(p_dat, bbframe_item_size, 1, fp);
	        if (items_read < 1)
	        {
	        	fseek(fp, 0, SEEK_SET);
	            printf ("Back to start of data file.\n");
	            items_read = fread(p_dat, bbframe_item_size, 1, fp);
	            if (items_read < 1)
	            {
	            	perror(NULL);
	            	printf ("There was an error with reading from the BBFrames file.\n");
	            	shutdown();
	            }
	        }

			// Schedule TX buffer
			nbytes_tx = iio_buffer_push_partial(txbuf, bbframe_item_size/p_inc);
			if (nbytes_tx != bbframe_item_size) { printf("Error pushing buf %d\n", (int) nbytes_tx); shutdown(); }
#endif
